...
const leapCheck = function checkForLeapYear(check) {
  if (check === 1) {
    if (
      (calLib.year % 4 === 0 && calLib.year % 100 !== 0) ||
      calLib.year % 400 === 0) {
...
function dateFull() {
  ...
  theDate.setFullYear(theDate.getFullYear());
  theDate.setMonth(0);
  theDate.setDate(1);
  ...
  for (i = 0; i < 60; i += 1) {
    ...
    calLib.push({
      date: new Date(((ref.getFullYear() - 30) + i), 0, 1),
...
// instaurer un system de conteur, start a la moitier de l'echelle du calendrier et permet de choisir l'année voulue
